...
class MAContinuousBurstCCNNetwork(nn.Module):
    def __init__(self, n_inputs, n_hidden_layers, n_hidden_units, n_outputs, p_baseline, tau_W, lr):
        ...
        tau_trace_list = [0.2 * (n_hidden_layers - i) for i in range(n_hidden_layers)] + [0.1]
        # print(tau_trace_list)
        self.tau_input_trace = tau_trace_list[0]

        if n_hidden_layers == 0:
            self.layers.append(MAContinuousBurstCCNOutputLayer(n_inputs, n_outputs, p_baseline, tau_W=tau_W, lr=lr))
        elif n_hidden_layers == 1:
            self.layers.append(MAContinuousBurstCCNHiddenLayer(n_inputs, n_hidden_units, n_outputs, p_baseline, tau_W=tau_W, lr=lr, tau_ma=tau_trace_list[1]))
            self.layers.append(MAContinuousBurstCCNOutputLayer(n_hidden_units, n_outputs, p_baseline, tau_W=tau_W, lr=lr))
        else:
            self.layers.append(MAContinuousBurstCCNHiddenLayer(n_inputs, n_hidden_units, n_hidden_units, p_baseline, tau_W=tau_W, lr=lr, tau_ma=tau_trace_list[1]))

            for i in range(1, n_hidden_layers - 1):
                self.layers.append(MAContinuousBurstCCNHiddenLayer(n_hidden_units, n_hidden_units, n_hidden_units, p_baseline, tau_W=tau_W, lr=lr, tau_ma=tau_trace_list[i+1]))

            self.layers.append(MAContinuousBurstCCNHiddenLayer(n_hidden_units, n_hidden_units, n_outputs, p_baseline, tau_W=tau_W, lr=lr, tau_ma=tau_trace_list[-1]))
            self.layers.append(MAContinuousBurstCCNOutputLayer(n_hidden_units, n_outputs, p_baseline, tau_W=tau_W, lr=lr))

        self.p_baseline = p_baseline

        self.register_buffer("input_event_rate_ma", torch.zeros(n_inputs, 1))

    def feedforward_only_update(self, input_event_rate, dt):
    ...
    def prediction_update(self, input_event_rate, dt):
        self.input_event_rate_ma = self.input_event_rate_ma + (dt / self.tau_input_trace) * (input_event_rate - self.input_event_rate_ma)
    ...
    def teaching_update(self, input_event_rate, target, dt):
        ...
        first_hidden_layer.weight_update(self.input_event_rate_ma, dt=dt)

        # for i, layer in list(enumerate(self.layers))[1:]:
        for i in range(1, len(self.layers)):
            self.layers[i].weight_update(self.layers[i-1].event_rate_ma_cache, dt=dt)

        # self.layers[-1].weight_update(self.layers[-2].event_rate_cache, dt=dt)
    ...
    def set_weights(self, layer_weights, layer_biases):
        for layer, weight, bias in zip(self.layers, layer_weights, layer_biases):
            ...
            layer.bias.data = bias.unsqueeze(1).clone()
...
class MAContinuousBurstCCNOutputLayer(nn.Module):
    def __init__(self, in_features, out_features, p_baseline, tau_W, lr):
        ...
        self.register_buffer('burst_prob', torch.ones(out_features, 1) * p_baseline)
        self.register_buffer('burst_prob_cache', torch.ones(out_features, 1) * p_baseline)
    ...
    def feedback_update(self, dt, target=None):
        ...
        # self.event_rate_ma = self.event_rate_ma + (dt / self.tau_ma) * (self.event_rate_cache - self.event_rate_ma)
        # self.burst_rate_ma = self.burst_rate_ma + (dt / self.tau_ma) * (self.burst_rate_cache - self.burst_rate_ma)
        # self.burst_prob_ma = self.burst_rate_ma / self.event_rate_ma

        # self.event_rate_ma.add_((dt / self.tau_ma) * (self.event_rate_cache - self.event_rate_ma))
        # self.burst_rate_ma.add_((dt / self.tau_ma) * (self.burst_rate_cache - self.burst_rate_ma))

        return self.event_rate_cache, self.burst_rate_cache

    def weight_update(self, input_event_rate_ma, dt):
        ...
        self.weight.add_(lr_factor * ((self.burst_prob_cache - self.p_baseline) * self.event_rate_cache).mm(input_event_rate_ma.t()))
        self.bias.add_(lr_factor * (self.burst_prob_cache - self.p_baseline) * self.event_rate_cache)


class MAContinuousBurstCCNHiddenLayer(nn.Module):
    def __init__(self, in_features, out_features, next_features, p_baseline, tau_W, lr, tau_ma):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
    ...
    def weight_update(self, input_event_rate_ma, dt=0.1):
